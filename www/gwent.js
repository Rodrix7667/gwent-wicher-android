...
class ControllerAI {
	...
	// Assigns a weight for how likely the controller is to activate its Habilidad de Líder
...
class Player {
	...
	canPlay() {
		return this.hand.cards.length > 0 || this.leaderdisponibles;
	}
	...
	// Disable access to Habilidad de Líder and toggles leader visuals to off state
	disableLeader(){
		this.leaderdisponibles = false;
	...
	// Enable access to Habilidad de Líder and toggles leader visuals to on state
	enableLeader() {
		this.leaderdisponibles = this.leader.activated.length > 0;
		let elem = this.elem_leader.cloneNode(true);
		this.elem_leader.parentNode.replaceChild(elem, this.elem_leader);
		this.elem_leader = elem;
...
class Game {
	...
	async initialRedraw(){
		...
		}, c => true, false, true, "Elige hasta 2 cartas para cambiar.");
	...
	exitGame()
	{
		...
		ui.popup(
			"Continuar", ()=>{},
			"Salir", ()=>this.returnToCustomization(),
			"¿Abandonar partida?" , "Esto te devolverá al menú de personalización de mazos."
		); 
	}
...
class Card {
	...
	constructor(card_data, player) {
		...
		if (this.row === "leader")
			this.desc_name = "Habilidad de Líder";
...
class UI {
	...
	// Initializes the local background music
	initYouTube(){
		this.youtube = document.getElementById('bg-music');
		if (!this.youtube) {
			this.youtube = document.createElement('audio');
			this.youtube.id = 'bg-music';
			this.youtube.loop = true;
			this.youtube.volume = 0.5;
			this.youtube.src = 'music/gwent_music.webm';
			document.body.appendChild(this.youtube);
		}
		this.ytActive = true;
		...
		if (!Settings.music.isEnabled()) {
			...
			this.youtube.play().catch(() => {});
		}
	}
	
	// Called when client toggles the music
	toggleMusic(){
		const isPlaying = this.youtube && !this.youtube.paused;
		if (isPlaying) {
			this.youtube.pause();
			this.toggleMusic_elem.classList.add("fade");
		} else {
	...
	setYouTubeEnabled(enable){
		if (this.ytActive === enable)
			return;
		if (enable && !this.mute && this.youtube)
			this.youtube.play().catch(() => {});
	...
	setDescription(card, desc){
		if (!card)
		{
			desc.children[1].innerHTML = "NULO";
...
class DeckMaker {
	...
	// Called when client selects a deck faction. Clears previous cards and makes valid cards disponibles.
	...
	startNewGame(){
		...
		if (this.stats.units < 22)
			warning += "Tu mazo debe tener al menos 22 cartas de unidad. \n";
		if (this.stats.special > 10)
			warning += "Tu mazo no debe tener más de 10 cartas especiales. \n";
		if (warning != "")
		{
			AudioManager.playSFX("warning");
		...
		player_me = new Player(0, "Jugador 1", me_deck);
		player_op = new Player(1, "Jugador 2", op_deck);
	...
	uploadDeck(id, callback)
	{
		...
		fr.onload = e => {
			try {
				...
				alert("¡El mazo subido no tiene el formato correcto!");
			}
			finally
			{
	...
	deckFromJSON(json) {
		...
		try {
			...
			AudioManager.playSFX('warning');
			alert("¡El mazo subido no se puede analizar!");
			return;
		}
	}
	...
	loadDeck(deck, siilent = true)
	{
		...
		if (deck.faction != card_dict[deck.leader].deck)
			warning += "El líder '" + card_dict[deck.leader].name + "' no coincide con la facción del mazo '" + deck.faction + "'.\n";
		// check if cards exist and have correct faction & count
		const cards = deck.cards.filter( c => {
			const card = card_dict[c[0]];
			if (!card) {
				warning += "ID " + c[0] + " no corresponde a una carta.\n";
				return false
			}
			if (![deck.faction, "neutral", "special", "weather"].includes(card.deck)) {
				warning += "'" + card.name + "' no puede usarse en un mazo de la facción '" + deck.faction +"'\n";
				return false;
			}
			if (card.count < c[1]) {
				warning += "El mazo contiene " + c[1] + "/" + card.count + " disponibles " + card_dict[c.index].name + " cards\n";
				c[1] = card.count;
				return true;
			}
			return true;
	...
	viewOponentCards()
	{
		...
		ui.queueCarousel(container, 1, ()=>{}, ()=>true, false, true, "Mazo del oponente");
	}
}
...
// Initializes music directly (no YouTube needed)
setTimeout(() => ui.initYouTube(), 1000);
